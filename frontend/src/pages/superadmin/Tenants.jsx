...
import { PLAN_ORDER, planName, planShortName } from '../../constants/plans'
...
export default function AdminTenants() {
  ...
  const handleDelete = (tenant) => {
    const ok = window.confirm(`سيتم حذف مركز "${tenant.name}" وكل بياناته نهائياً. هل أنت متأكد؟`)
    if (ok) remove.mutate(tenant.id)
  }
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-emerald-700">
                كود التفعيل للمدير: <span className="font-mono font-bold text-lg tracking-widest">{activationResult.activation_code}</span>
              </p>
              <p className="mt-1 text-sm text-slate-600">
                الواتساب: {activationResult.whatsapp_status === 'sent' ? 'تم الإرسال' : activationResult.whatsapp_status === 'skipped' ? 'لم يُرسل (لا يوجد رقم)' : 'فشل الإرسال — شارك الكود يدويًا'}
              </p>
              <p className="mt-1 text-xs text-slate-500">رابط التفعيل: carecar.online/activate</p>
            </div>
            <button onClick={() => setActivationResult(null)} className="text-slate-400 hover:text-slate-700 text-lg font-bold leading-none">×</button>
          </div>
        </div>
      )}
        ...
        <div className="surface mb-6 rounded-lg p-6">
          <div className="mb-5 flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">سيصل كود التفعيل للمدير عبر الواتساب لإعداد كلمة المرور.</p>
            </div>
          </div>
          <div className="grid gap-4 lg:grid-cols-3">
            {[
              ...
              ['manager_phone', 'هاتف المدير (واتساب)', 'text'],
          ...
          <tbody>
            {[...tenants].sort((a, b) => Number(a.is_active) - Number(b.is_active) || a.name.localeCompare(b.name, 'ar')).map(t => (
              <tr key={t.id} className={`border-t border-slate-100 text-slate-700 hover:bg-slate-50 ${!t.is_active ? 'bg-rose-50/45' : ''}`}>
                ...
                <td className="px-5 py-4"><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-bold">{planShortName(t.plan)}</span></td>
